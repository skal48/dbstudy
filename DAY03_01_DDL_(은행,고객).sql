...
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2(6BYTE) CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE'); --글자를 쓸때는 작은 따옴표('') 쓰기
ALTER TABLE CUSTOMER_TBL ADD GRADE VARCHAR2(6BYTE) CHECK(GRADE IN ('VIP', 'GOLD', 'SILVER', 'BRONZE')); --IN 으로 하면 FOR 문으로 쓸 수 있으나 OR로 하면 안됨

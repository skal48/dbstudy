...
/*
    데이터 타입
    1. NUMBER (p,s) :정밀도(p), 스케일(s)로 표현하는 숫자 형식
        1) 정밀도 p : 전체 유효 숫자(의미 있는 숫자)의 갯수
        2) 스케일 s : 소수부의 유효 숫자의 갯수 
    2. CHAR(size) : character(문자), 고정 문자  
        1)글자 수가 고정된 타입(예시 : 핸드폰 번호, 주민번호등)
        2)size : 최대 2000 byte
    3. VARCHAR2(size) : 가변 문자 
        1)글자 수가 고정되지 않은 타입(예시 : 이름, 이메일, 주소 등)
        2)size : 최대 4000 byte
    4. CLOB : 큰 텍스트 타입(최대 4G)
...
CREATE TABLE CUSTOMER_T(
    ...
    JUBUN   CHAR(14 byte) NOT NULL UNIQUE
);

...
CREATE OR REPLACE PROCEDURE MY_PROC_03(EMPLOYEEID IN EMPLOYEES.EMPLOYEE_ID%TYPE)  --프로시저로 전달된 인수를 저장하는 입력파라미터(변수)  (오라클에만 입력파라미터가 있다. )
IS                                                --IN 생략가능                      
    FIRSTNAME EMPLOYEES.FIRST_NAME%TYPE;
    LASTNAME  EMPLOYEES.LAST_NAME%TYPE;
BEGIN
...
EXECUTE MY_PROC_03(206); -- 프로시저로 전달하는 인수 206이라고 

-- 프로시저로부터 값을 받아오는 방법
-- 사원 번호가 100인 사원의 이름을 받아온는 프로시저 
CREATE OR REPLACE PROCEDURE MY_PROC_04(FNAME OUT EMPLOYEES.FIRST_NAME%TYPE,LNAME OUT EMPLOYEES.LAST_NAME%TYPE) --FNAME, LNAME.은 프로시저 외부로 값을 반환하는 출력 파라미터 
AS --변수 선언안해도 써야함  
BEGIN
    SELECT FIRST_NAME, LAST_NAME
      INTO FNAME, LNAME
...
DECLARE
    ...
    LASTNAME EMPLOYEES.LAST_NAME%TYPE;
BEGIN
    MY_PROC_04(FIRSTNAME, LASTNAME);  -- FIRSTNAME, LASTNAME 변수에 결과를 저장해 달라. PLSQL에서 직접프로시저를 호출할 때 EXECUTE을 생략한다.  
    DBMS_OUTPUT.PUT_LINE(FIRSTNAME || ' '|| LASTNAME);
END;
...
/*
    ...
    3. 프로시저 호출의 예시
        BUY_PROC(1, 1000,10);  고객번호 1이 제품코드 100을 10개 구매하였다. 
...
BEGIN
    ...
    -- 2) 고객 테이블의 포인트를 수정한다.( 구매액의 10% 소수이하 올림 처리 )
    UPDATE CUST_T
       SET C_POINT = C_POINT + CEIL(AMOUNT * (SELECT P_PRICE FROM PROD_T WHERE P_CODE = PCODE) * 0.1)
     WHERE C_NO = CNO;
     
    -- 3) 주문 테이블의 주문내역을 삽입한다. 
    INSERT INTO BUY_T(B_NO, C_NO, P_CODE, B_AMOUNT) 
    VALUES (BUY_SEQ.NEXTVAL, CNO, PCODE, AMOUNT);
    
    -- 4) 커밋
    COMMIT;
EXCEPTION

    WHEN OTHERS THEN  --모든예외

...
INSERT INTO CUSTOMER_T(CUST_ID, CUST_NAME, CUST_ADDR, CUST_TEL) VALUES(CUST_SEQ.NEXTVAL, '김연아', '대한민국', '111-111-11');
...
UPDATE ORDER_T
   SET ORDER_ID = TO_CHAR(ORDER_DT,'YYMMDD') ||'-'|| ORDER_ID ;    -- CONCAT(TO_CHAR(ODER_DT,'YYMMDD-'), ORDER_ID)
...
SELECT *
 FROM (SELECT RANK() OVER(ORDER BY PRICE DESC) AS 순위
     , BOOK_ID, BOOK_NAME, PUBLISHER, PRICE FROM BOOK_T)
 WHERE 순위 = 1;
...
SELECT O.ORDER_ID    AS 주문번호
 ...
 ORDER BY O.ORDER_DT;
-- 날짜 연산에는 TO_DATE이용해서 하는 것이 안전하다 써먹어라 
...
SELECT O.ORDER_ID           AS 구매번호
 ...
 WHERE ORDER_DT NOT BETWEEN TO_DATE('20/07/04') AND TO_DATE('20/07/07');
...
SELECT NVL(O.ORDER_ID, 'NULL') AS 주문번호
     , B.BOOK_ID               AS 책번호
     , B.BOOK_NAME             AS 책이름
...
SELECT NVL(C.CUST_NAME,'NULL')             AS 고객명
     , B.BOOK_NAME                         AS 책이름
     , B.PRICE                             AS 책가격
  FROM ORDER_T O INNER JOIN CUSTOMER_T C
...
SELECT C.CUST_NAME    AS 고객명
     , SUM(O.AMOUNT)  AS 구매도서수
  FROM ORDER_T O INNER JOIN CUSTOMER_T C
    ON O.CUST_ID = C.CUST_ID                 
 WHERE CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, CUST_NAME ;  -- 고객이름이 같을 수가 있기에 ID 넣어준다.
...
SELECT C.CUST_NAME                 AS 고객명
     , COUNT(DISTINCT B.PUBLISHER) AS 출판사수  --같은 이름의 출판사가 중복될수 있어서  
  FROM ORDER_T O INNER JOIN CUSTOMER_T C
    ON O.CUST_ID = C.CUST_ID INNER JOIN BOOK_T B
    ON O.BOOK_ID = B.BOOK_ID
...
-- 19. 모든 구매 고객의 이름과 총구매액(PRICE * AMOUNT)과 구매횟수를 조회하시오. 구매 이력이 없는 고객은 총구매액과 구매횟수를 
-- 0으로 조회하고, 고객번호 오름차순으로 정렬하시오.
SELECT CUST_NAME AS 고객명
     , NS AS 총구매액
     , CA AS 구매횟수
  FROM (SELECT CUST_NAME, C.CUST_ID, NVL(SUM(B.PRICE * O.AMOUNT),0) AS NS, COUNT(O.AMOUNT) AS CA
          FROM ORDER_T O  RIGHT OUTER JOIN CUSTOMER_T C
...
CREATE VIEW V_TRIPLE AS(
SELECT C.CUST_NAME,SUM(PRICE * AMOUNT) AS PA 
          FROM ORDER_T O INNER JOIN CUSTOMER_T C
            ON O.CUST_ID = C.CUST_ID INNER JOIN BOOK_T B
            ON O.BOOK_ID = B.BOOK_ID 
            GROUP BY C.CUST_NAME);
...
SELECT CUST_NAME AS 고객명
     , PA AS 총구매액
  FROM V_TRIPLE
    WHERE PA NOT IN((SELECT MAX(PA) FROM V_TRIPLE) ,(SELECT MIN(PA) FROM V_TRIPLE))
    ORDER BY PA DESC;

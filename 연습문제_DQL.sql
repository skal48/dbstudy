...
SELECT EMPLOYEE_ID
     , NVL(SALARY * COMMISSION_PCT, 0) AS COMMISSION      --FROM EMPLOYEES WHERE COMMISSION_PCT IS NOT NULL;
  FROM EMPLOYEES;
  
-- 9. EMPLOYEES 테이블에서 FIRST_NAME이 'J'로 시작하는 사원을 조회하시오.
SELECT *
  FROM EMPLOYEES
 WHERE SUBSTR(FIRST_NAME,1,1) = 'J';  --FIRST_NAME LIKE 'j%'
 
-- 10. EMPLOYEES 테이블에서 JOB_ID가 'MAN'으로 끝나는 사원들의 MANAGER_ID를 중복을 제거하여 조회하시오.
SELECT DISTINCT MANAGER_ID
  FROM EMPLOYEES
 WHERE SUBSTR(JOB_ID,LENGTH(JOB_ID)-2,3) = 'MAN';    -- JOB_ID LIKE '&MAN'
 
-- 11. EMPLOYEES 테이블에서 전체 사원을 DEPARTMENT_ID의 오름차순으로 조회하되, 동일한 DEPARTMENT_ID 내에서는 HIRE_DATE의 오름차순으로 조회하시오.
SELECT *
  FROM EMPLOYEES
 ORDER BY DEPARTMENT_ID ASC, HIRE_DATE ASC;   -- TODATE(HIRE_DATE, YY/MM/DD') ASC
...
SELECT EMPLOYEE_ID
     , CEIL(MONTHS_BETWEEN(SYSDATE, TO_DATE(HIRE_DATE, 'YY/MM/DD'))) AS 근무개월수  --CEIL(MONTH_BETWEEN(SYSDATE,HIRE_DATE)) || '개월' AS 근무개월수
  FROM EMPLOYEES;
...
HAVING SUBSTR(DEPARTMENT_NAME,1,2) IN('IT', 'Co');

WHERE 과 HAVING 에 쓰는 차이는 무엇인가.?

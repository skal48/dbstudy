...
*/
--서버 메세지 출력 ON  꼭 먼저 해줘야 아래가 실행된다. 
SET SERVEROUTPUT ON;
...
BEGIN
    SELECT EMPLOYEE_ID, FIRST_NAME, LAST_NAME
      ...
      FROM EMPLOYEES
     WHERE EMPLOYEE_ID = 100;   -- 조회한 내용을 각각 변수에 저장
...
BEGIN
    ...
    WHILE EMPLOYEEID <= 109 LOOP
      SELECT EMPLOYEE_ID, LAST_NAME, FIRST_NAME
        INTO EMPLOYEEID, LASTNAME, FIRSTNAME
        FROM EMPLOYEES
        WHERE EMPLOYEE_ID = EMPLOYEEID;
...
--1 부터 누적하기 . 누적 결과가 100을 넘어가면 그만 누적하고 출력하기 
DECLARE
    N NUMBER;
    TOTAL NUMBER;
BEGIN
...
BEGIN
    ...
    WHILE TRUE LOOP
        -- N의 증가 
        N := N + 1;
        --누적결과가 100이 넘어가면 그만하기 
        IF TOTAL > 100 THEN
            EXIT;
        END IF;
        -- 3의 배수 누적해서 제외하기
        SELECT MOD(N, 3) INTO MODULAR  --3으로 나눈 나머지는 모듈라 변수에 저장
          FROM DUAL;
        IF MODULAR = 0 THEN
...
END;   
  ...
  --BEGIN 의 마지막에는 COMMIT이 되고 EXCEPTION 에는 ROLLBACK이 있다. 

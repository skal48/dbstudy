...
SELECT USER_NO
     ...
     , USER_MOBILE2
  FROM USER_T
 WHERE SUBSTR(USER_MOBILE2,1,1) = 5;   --USER_MOBILE2 LIKE '5%'
...
SELECT USER_NO
     ...
     , USER_REGDATE
  FROM USER_T
 WHERE TO_CHAR(USER_REGDATE, 'YYYY') > '2010';  --EXTRACT(YEAR FROM USER_REGDATE) >= 2010    날짜 추출 함수 

-- 4. 사용자번호와 연락처1, 연락처2를 연결하여 조회하시오. 연락처가 없는 경우 NULL 대신 'None'으로 조회하시오.
SELECT USER_NO
     , NVL(USER_MOBILE1 || USER_MOBILE2, 'None') AS USER_PHONE  --NVL( CONCAT(USER_MOBILE1,USER_MOBILE2), 'None')
 FROM USER_T;
...
SELECT USER_ADDR AS 주소
     , COUNT(*)  AS 사용자수
  FROM USER_T
 WHERE USER_ADDR NOT IN('서울', '경기')    --USER_ADDR != '서울' AND USER_ADDR != '경기'
 GROUP BY USER_ADDR;

--그룹핑 이전에 처리 할 수 있으면 WHERE
--그룹핑 이후에는  HAVING  (대표적으로 그룹핑에서 계산 한 이후에 지역별 사용자수가 2명 이상인)
...
-- 7. 구매내역이 없는 사용자를 조회하시오. (조인과 서브쿼리로 처리가능)
...
SELECT U.USER_ID AS 아이디
  ...
  FROM BUY_T B RIGHT OUTER JOIN USER_T U  
    ON B.USER_NO = U.USER_NO
 GROUP BY USER_ID, USER_NAME   --멀티그룹 두가지가 들어가야함 그룹바이에서 지정한 것 만 조회가능 ,아이디 이름 둘 다 들어가 있어야함 
 ORDER BY USER_ID;
...
SELECT PROD_NAME AS 제품명
     , CONCAT(CTN, '개') AS 판매횟수
    FROM (SELECT COUNT(BUY_AMOUNT) AS CTN
               , PROD_NAME 
...
SELECT P.PROD_NAME AS  제품명
 ...
 FROM PRODUCT_T P LEFT OUTER JOIN BUY_T B
   ON P.PROD_CODE = B.PROD_CODE
GROUP BY P.PROD_CODE, P.PROD_NAME;   --코드와 이름이 동일한 데이터는 한개로 처리  안전장치를 걸음// 동일제품이나 이름이 같을 경우 같은제품으로 보는 경우가 있음 
...
SELECT U.USER_NAME AS 고객명
     , P.PROD_NAME AS 제품명
     , (PROD_PRICE * BUY_AMOUNT) AS 구매액
  FROM BUY_T B INNER JOIN PRODUCT_T P 
    ON B.PROD_CODE = P.PROD_CODE INNER JOIN USER_T U
    ON B.USER_NO = U.USER_NO
...
SELECT USER_ID AS 아이디
 ...
 FROM (SELECT U.USER_ID, U.USER_NAME
            , COUNT(B.BUY_NO) AS BN
            , SUM(B.BUY_AMOUNT * P.PROD_PRICE) AS AMO
         FROM BUY_T B INNER JOIN USER_T U
           ON B.USER_NO = U.USER_NO LEFT OUTER JOIN PRODUCT_T P
           ON B.PROD_CODE = P.PROD_CODE
...
DELETE 
 ...
 WHERE BUY_NO = (SELECT BUY_SEQ.CURRVAL
                   FROM DUAL);   --권장하지는 않음(비추천) 시퀀스는 사용하였으나 INSERT의 자체가 실패한 경우 가장 큰 구매번호와 CURRVAL값은 다르다. 
...
DELETE 
  FROM USER_T
 WHERE USER_NO = 5;    --PK와 FK의 참조무결성에 위배되기 때문이다. 